...
echo =======================================================
echo Starting Paper Agent services
echo =======================================================
...
if not exist "%PYTHON_ENV%" (
  ...
  echo Run setup_new_computer.bat first.
  pause
  exit /b 1
)
...
echo [0/5] Stopping previous project services...
...
echo [4/5] Starting cloud backend on port 7007...
cd /d "%CLOUD_DIR%"
wscript //nologo "%EDGE_DIR%\run_silent.vbs" "cmd /d /c set GRADIO_ROOT_PATH=/cloud&& ""%PYTHON_ENV%"" -u app.py --server-name 0.0.0.0 --server-port 7007 > %LOG_DIR%\cloud_backend.log 2>&1"
